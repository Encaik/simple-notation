...
export class ChordTool {
  static pianoChordMap: Record<string, string[]> = {
    // 大三和弦
    C: ['C3', 'E3', 'G3'],
    D: ['D3', 'F#3', 'A3'],
    E: ['E3', 'G#3', 'B3'],
  ...
  static guitarChordPositionsMap: Record<string, (number | null)[]> = {
    C: [null, 3, 2, 0, 1, 0], // C
   ...
   * E2 A2 D3 G3 B3 E4
   */
  guitarTuningMidis: Record<number, number> = {
    6: 40, // E2
    5: 45, // A2
    4: 50, // D3
    3: 55, // G3
   ...
   */
  static getPianoNotesForChord(chordSymbol: string): string[] {
    return ChordTool.pianoChordMap[chordSymbol] || [];
  }
   ...
   */
  static getGuitarPositionsForChord(chordSymbol: string): (number | null)[] {
   ...
   */
  static getMidiForGuitarPosition(string: number, fret: number): number | null {
    const openStringMidi = SNTransition.guitarTuningMidiMap[string];
   ...
   */
  static getNoteNameForGuitarPosition(
    string: number,
    fret: number,
  ): string | null {
    const midi = ChordTool.getMidiForGuitarPosition(string, fret);
    if (midi === null) {
      return null;
    }
