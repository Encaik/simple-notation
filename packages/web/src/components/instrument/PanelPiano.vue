  ...
  >
    ...
    >
      ...
      <div
        ...
        :class="[
          pianoStore.highlightMidis.includes(key.midi) || tempHighlightedKeys[key.midi]
            ? 'bg-[#ffe082]'
            : 'bg-white',
        ]"
...
// 88键钢琴的音名和黑白键分布
const keyPattern = ['A', 'A#', 'B', 'C', 'C#', 'D', 'D#', 'E', 'F', 'F#', 'G', 'G#'];
...
function getBlackKeyStyle(key: PianoKey) {
  ...
  // 黑键左侧的白键数量
  const leftWhiteCount = pianoStore.keys.slice(0, idx).filter((k) => k.type === 'white').length;
  const whiteCount = pianoStore.whiteKeys.length;
  const keyWidth = 100 / whiteCount;
  // 黑键宽度为白键的60%
  return {
...
async function handleKeyClick(noteName: string, midi: number) {
  ...
  try {
    ...
    // For manual clicks, treat as melody highlight
    pianoStore.setHighlightMidis([midi], 'melody');
    setTimeout(() => {
      // Clear only melody highlights after timeout
...
async function handleTouchMove(event: TouchEvent) {
  if (isDragging.value && event.touches.length > 0) {
    ...
    if (
      ...
      const keyIndex = parseInt(targetElement.getAttribute('data-key-index') || '-1', 10);
      if (keyIndex !== -1) {
