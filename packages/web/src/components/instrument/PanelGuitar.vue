  ...
  >
    ...
    >
      <div class="flex-1 grid grid-cols-layout h-full">
        <div class="col-start-1 col-end-2 grid grid-rows-6 h-full border-r border-[#bbb] bg-[#eee]">
          <div
        ...
        >
          <div class="grid grid-rows-6 h-full" :style="gridTemplateColumns">
            ...
            <template v-for="stringIndex in 6" :key="`string-row-${stringIndex}`">
              <div
...
const gridTemplateColumns = computed(() => {
  ...
  const totalFrettedLength = fretPositions.value[numFrets] - fretPositions.value[0];
...
const allHighlightPositions = computed(() => {
  ...
  return [...Object.values(combined), ...Object.values(tempHighlightedPositions.value)];
});
...
const capoStyle = computed<CSSProperties>(() => {
  ...
  const spaceCenterPercentage = (spaceLeftPercentage + spaceRightPercentage) / 2; // 空间中心百分比

  // 计算变调夹的左侧位置，使其中心对齐到空间中心
  const leftPositionPercentage = spaceCenterPercentage - capoWidthPercentage / 2;
 ...
 * @returns {object}
 */
function getFretDotStyle(fret: number, type: 'single' | 'double-top' | 'double-bottom') {
 ...
 * @returns {CSSProperties}
 */
function getHighlightDotStyle(stringIndex: number, fret: number): CSSProperties {
  if (fret === 0) {
    ...
    const spaceCenterPercentage = (spaceLeftPercentage + spaceRightPercentage) / 2;
...
async function handleGuitarPositionClick(stringIndex: number, fret: number) {
  ...
  if (noteName) {
    ...
    guitarStore.setHighlightPositions([{ string: stringIndex, fret: fret }], 'melody');
    setTimeout(() => {
 ...
 * @returns {void}
 */
async function handleGuitarPositionMouseOver(stringIndex: number, fret: number) {
...
async function handleTouchMove(event: TouchEvent) {
  if (isDragging.value && event.touches.length > 0) {
    ...
    if (
      ...
      const stringIndex = parseInt(targetElement.getAttribute('data-string-index') || '-1', 10);
      const fret = parseInt(targetElement.getAttribute('data-fret-index') || '-1', 10);
