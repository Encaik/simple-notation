...
import { ChordTool } from 'simple-notation';
...
export const usePianoStore = defineStore('piano', () => {
  ...
  // 白键和黑键分组
  const whiteKeys = computed(() => keys.value.filter((k) => k.type === 'white'));
  const blackKeys = computed(() => keys.value.filter((k) => k.type === 'black'));
   ...
   * @returns {void}
   */
  function setHighlightMidis(keyMidis: number[], type: 'melody' | 'chord' = 'melody') {
    if (type === 'melody') {
