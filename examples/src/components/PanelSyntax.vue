  ...
  >
    ...
    <div class="bg-[#f9f9f9] p-4 rounded-md mb-6">
      ...
      <ul class="list-disc pl-6">
        ...
        <li>
          <strong>附点</strong>：使用 `.` 表示附点，例如 `/2.`
          表示附点二分音符。需要注意的是，附点要加在时值后面，例如 `4/8.`
          表示附点八分音符。
        </li>
        <li>
        ...
        <li>
          <strong>升降号</strong>：使用 `#` 表示升号，`b`
          表示降号。多个升降号时，会使用重升号和重降号，例如 `##3`
          表示升两个半音的音符 `3`。
        </li>
        <li>
          <strong>八度升降</strong>：使用 `^` 表示升八度，`_`
          表示降八度。数字为几就表示升降几个八度，例如 `3^^`
          表示升高两个八度的音符 `3`。
        </li>
        <li>
          <strong>括号</strong
          >：括号一般用于特殊音符组合，在实际使用中可根据具体需求对音符进行分组，例如
          `(3,4)` 表示特定的音符组合，常用于前奏间奏等部分的标记。
        </li>
        <li>
          <strong>连音线</strong>：使用方括号 `[` 和 `]`
          表示连音线的开始和结束，例如 `[3,4,5]` 表示 `3`、`4`、`5`
          这几个音符用连音线连接。
        </li>
        <li>
          <strong>装饰音</strong>：使用尖括号 `<` 和 `>`
        ...
        <li>
          <strong>重复记号</strong>：在小节线 `|` 旁边使用 `:` 表示重复。`:`
          在小节线左侧（例如 `|:`）表示重复开始，在右侧（例如
          `:|`）表示重复结束。`|:` 和 `:|`
          可以同时使用表示重复一个小节或一段。示例：`|: 1 2 | 3 4 :|` 表示重复"1
          2"和"3 4"这两个小节。
        </li>
      </ul>
      <p>示例：</p>
...
2,2,1,-|5,5,4,4|3,3,2,-</code></pre>
    ...
    <div class="bg-[#f9f9f9] p-4 rounded-md">
      ...
      <p>歌词中`-`代表跳过当前的音符，不为其分配歌词。</p>
      <p>
        <strong>括号</strong>：在歌词中使用 `()` 或 `（）`
        将内容包裹起来，这部分内容会被视为一个整体与简谱中的一个音符对应。例如
        `轻轻地(我将)离开你` 中，`(我将)` 会被视为一个歌词单元与一个音符对应。
      </p>
      <p>
        <strong>竖排多行歌词</strong>：使用 `[数字.内容]`
        格式可以编写竖排的多行歌词。其中 `数字` 表示行号（从1开始），`内容`
        是该行对应的歌词片段。连续的 `[数字.内容]`
        块会合并成一个竖排歌词单元。每个 `内容`
        部分会按照单字、英文单词、括号等规则拆分。示例：`[1.红][2.黄][3.蓝]`
        会在同一个音符下方显示"红"、"黄"、"蓝"三行字。
      </p>
      <p>示例：</p>
      <pre
        class="bg-white p-2 border border-[#ddd] rounded-md"
